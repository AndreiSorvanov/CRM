...
(() => {
  ...
  const contactIconsPath = 'crm-frontend/img/contacts-sprite.svg';
  let sort = {
    col: 'id',
    asc: true,
  };
  ...
  async function openEditClientModal(clientId) {
    ...
    const editClientModal = createEditClientModal(clientId, clientData, { onEdit: onEditClient, onDelete: onDeleteClient });
  ...
  function deleteClientHandler() {
    ...
    return (e) => {
      ...
      const deleteClientModal = createDeleteClientModal(clientId, { onDelete: onDeleteClient });
  ...
  function transformPhoneNumber(numberString) {
    return `+7 (${numberString.substring(0, 3)}) ${numberString.substring(3, 6)}-${numberString.substring(6, 8)}-${numberString.substring(8, 10)}`;
  }
  ...
  function createContactIcon(contactType) {
    ...
    const contactIcon = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    contactIcon.classList.add('contact__icon');
    contactIcon.setAttribute('xmlns', 'http://www.w3.org/2000/svg');

    const contactIconUse = document.createElementNS('http://www.w3.org/2000/svg', 'use');
  ...
  function createContactItem(contact) {
    ...
    contactBtn.setAttribute('aria-label', `${contact.type}: ${getTooltipContactValue(contact)}`);
  ...
  function createContactsList(contacts) {
    ...
    contactsCell.classList.add('clients-table__body-cell', 'clients-table__contacts-col', 'clients-table__body-contacts-col');
  ...
  function createActionsCell() {
    ...
    actionsCell.classList.add('clients-table__body-cell', 'clients-table__actions-col', 'clients-table__body-actions-col');
  ...
  function createTableRow(client) {
    ...
    idCell.classList.add('clients-table__body-cell', 'clients-table__id-col', 'clients-table__body-id-col');

    const fullnameCell = document.createElement('td');
    fullnameCell.innerText = `${client.surname} ${client.name} ${client.lastName}`;
    fullnameCell.classList.add('clients-table__body-cell', 'clients-table__fullname-col', 'clients-table__body-fullname-col');

    const creationDateCell = createDateAndTimeCell(
      new Date(client.createdAt),
      ['clients-table__body-cell', 'clients-table__creation-date-col', 'clients-table__body-creation-date-col'],
      ['creation-date'],
      ['creation-time']
    );
    ...
    const lastEditCell = createDateAndTimeCell(
      ...
      ['clients-table__body-cell', 'clients-table__last-edit-date-col', 'clients-table__body-last-edit-date-col'],
      ['last-edit-date'],
      ['last-edit-time']
    );
    ...
    tableRow.append(idCell, fullnameCell, creationDateCell, lastEditCell, contactsCell, actionsCell);
  ...
  function disposeTooltips(tableBodyId) {
    ...
    const tooltipsElems = tableBody.querySelectorAll('span[data-bs-toggle="tooltip"]');
    const tooltips = Array.from(tooltipsElems).map((tooltip) => bootstrap.Tooltip.getInstance(tooltip));
  ...
  function showTable(tableBodyId, clients) {
    ...
    const tooltipTriggerList = [].slice.call(document.querySelectorAll('span[data-bs-toggle="tooltip"]'))
  ...
  function sortClients(clients, col, asc) {
    ...
    switch (col) {
      case 'id':
        return [...clients].sort((a, b) => (Number(a.id) - Number(b.id)) * sign);
      case 'fullname':
  ...
  function highlightTableRow(tableBodyId, clientId) {
    const tableBody = document.getElementById(tableBodyId);
    const tableRows = Array.from(tableBody.querySelectorAll('.clients-table__body-row'));
  ...
  function createSearchListItem(client, search) {
    ...
    let itemText = `${client.surname} ${client.name} ${client?.lastName}`.trim();
    const startIndex = itemText.toLowerCase().indexOf(search);
    if (startIndex !== -1) {
      const endIndex = startIndex + search.length;
      itemText = itemText.slice(0, startIndex) + `<span class="search-list-item__found">${itemText.slice(startIndex, endIndex)}</span>` + itemText.slice(endIndex);
    }
    else {
      for (let contact of client.contacts) {
        ...
        if (startIndex !== -1) {
          const endIndex = startIndex + search.length;
          let foundContactValue = contact.value.slice(0, startIndex) + `<span class="search-list-item__found">${contact.value.slice(startIndex, endIndex)}</span>` + contact.
  ...
  async function showSearchList(search) {
    ...
    if (search === '') {
      ...
      if (response.status === 'OK') {
        ...
        if (clients.length > 0) {
          searchResultList.append(...clients.map((client) => createSearchListItem(client, search)))
        }
      }
    }
  }
  ...
  function pickSearchListItem(searchListItem, tableBodyId) {
    ...
    const highlitedRow = document.querySelector('.clients-table__body-row_found');
  ...
  document.addEventListener('DOMContentLoaded', async (e) => {
    ...
    addClientBtn.addEventListener('click', (e) => {
      const createClientModal = createCreateClientModal({ onCreate: onCreateClient });
      createClientModal.show();
    });

    const sortBtns = document.querySelectorAll('.clients-table-head__toggle-sort-btn');
    ...
    searchInput.addEventListener('keydown', (e) => {
      const searchListItems = Array.from(document.querySelectorAll('.header__search-list-item'));
      let index = searchListItems.findIndex((item) => item.getAttribute('aria-selected'));

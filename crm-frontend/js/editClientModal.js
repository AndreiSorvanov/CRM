import { createClientModalHTML, createClientModalBodyHTML, createClientModal, getClientDataFromModal, validate, showErrorMessages, blockModal, unblockModal } from './clientModal.js';
...
function createEditClientModalHTML(title, clientId) {
  ...
  const footerContainerElement = editClientModalElement.querySelector('.modal__footer > .modal__container');
...
function showLoader(modalElement) {
  ...
  const loaderContainer = modalElement.querySelector('.modal__confirm-btn-icon-container');
...
export function createEditClientModal(clientId, clientData, { onEdit = (e) => {}, onDelete = (e) => {}, onCancel = (e) => {}} = {}) {
  const editClientModalElement = createEditClientModalHTML('Изменить данные', clientId);
  ...
  const onDeleteClient = async (e) => {
    ...
    const onCancelDelete = (e) => {
      if (!isDeleted) {
        const editClientModal = createEditClientModal(clientId, clientData, { onEdit, onDelete });
        editClientModal.show();
      }
    }

    const deleteClientModal = createDeleteClientModal(clientId, { onDelete: onSubmitDelete, onCancel: onCancelDelete });

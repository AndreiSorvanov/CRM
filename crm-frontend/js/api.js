import { GET_CLIENTS_PATH, get_client_by_id_path, CREATE_CLIENT_PATH, get_edit_client_path, get_delete_client_path } from './config.js';
...
const fromAppToDbContactsTypeDict = new Map(Array.from(fromDBToAppContactsTypeDict, arr => arr.reverse()));
...
function transformClientData(clientData) {
  return {
    ...
    contacts: clientData.contacts.map(({ type, value }) => ({ type: fromDBToAppContactsTypeDict.get(type), value })),
  };
}

export async function getClientsFromDB(search = null) {
  try {
    const response = await fetch(GET_CLIENTS_PATH + (search ? `?search=${search}` : ''));
...
export async function createClientInDB(clientData) {
  try {
    const client = {
      ...clientData,
      contacts: clientData.contacts.map(({ type, value }) => ({ type: fromAppToDbContactsTypeDict.get(type), value })),
    }

...
const modalIconsPath = 'crm-frontend/img/modal-sprite.svg';
let isBlocked = false;
...
function createDropdown(list) {
  ...
  const dropdownBtnIconSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  ...
  const dropdownBtnIconUse = document.createElementNS('http://www.w3.org/2000/svg', 'use');
...
function chooseDropdownValue(dropdownElement, value) {
  ...
  dropdownItemsList.forEach((item) => item.setAttribute('class', 'dropdown-item dropdown__item'));

  const dropdownActiveItem = dropdownItemsList.filter(item => item.innerText === value)[0];
  dropdownActiveItem.classList.add('dropdown__item_active');
  dropdownActiveItem.setAttribute('aria-current', 'true');

  const nonactiveDropdownItemsList = dropdownItemsList.filter(item => !item.classList.contains('dropdown__item_active'));
  nonactiveDropdownItemsList[0].classList.add('dropdown__item_first');
  nonactiveDropdownItemsList[nonactiveDropdownItemsList.length - 1].classList.add('dropdown__item_last');
...
function createClientContactItemDeleteButton(onClick) {
  ...
  deleteContactBtnIconContainer.classList.add('contact-edit__delete-btn-icon-container');
  deleteContactBtn.append(deleteContactBtnIconContainer);

  const deleteContactBtnIcon = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  ...
  const deleteContactBtnIconUse = document.createElementNS('http://www.w3.org/2000/svg', 'use');
...
export function getClientDataFromModal(clientModalElement) {
  ...
  const contactsList = clientModalElement.querySelectorAll('.contacts-section__item');
  data.contacts = [];
  for (let contactItem of contactsList) {
    ...
    const contactValueElement = contactItem.querySelector('.contact-edit__input');
...
function addContact(clientModalElement, { contactType = '', contactValue = '' } = {}) {
  const contactsSection = clientModalElement.querySelector('.contacts-section');
  const contactsList = clientModalElement.querySelector('#create-edit-client-modal-contacts-list');
  const addContactBtn = clientModalElement.querySelector('#create-edit-client-modal-add-contact-btn');
...
export function validate(clientModalElement) {
  ...
  const invalids = [];
  if (clientData.name === '') invalids.push({ field: 'name', message: 'Не указано имя' });
  if (clientData.surname === '') invalids.push({ field: 'surname', message: 'Не указана фамилия' });
  clientData.contacts.map((contact, index) => {
    if (contact.value === '') {
      invalids.push({ field: 'contacts', index, message: 'Контакт заполнен не полностью' });
    }
  });
  clientData.contacts.map((contact, index) => {
    if (contact.type === 'Телефон' && contact.value !== '' && validatePhone(contact.value)) {
      invalids.push({ field: 'contacts', index, message: 'Телефон заполнен не полностью' });
    }
  });
  clientData.contacts.map((contact, index) => {
    if (contact.type === 'Email' && contact.value !== '' && !validateEmail(contact.value)) {
      invalids.push({ field: 'contacts', index, message: 'Email имеет неверный формат' });
    }
  });
...
export function blockModal(clientModalElement, onBlock = () => {}, blockClosing = false) {
  isBlocked = blockClosing;

  const fullnameSectionInputs = clientModalElement.querySelectorAll('.fullname-section__input');
  fullnameSectionInputs.forEach((input) => {
    input.readOnly = true;
    input.disabled = true;
  });

  const contactContainers = clientModalElement.querySelectorAll('.contact-edit');
...
export function createClientModal(clientModalElement, { clientData = {}, onCancel = (e) => {}, onBlock = () => {} } = {}) {
